<template>
  <div class="order-export-task">
    ...
    <el-table :data="taskData.beanList" style="width: 100%" v-loading="loading" element-loading-text="拼命加载中">
      <el-table-column prop="fileName" label="文件名称" :show-overflow-tooltip="true" min-width="250">
      </el-table-column>
      <el-table-column prop="createDate" label="操作时间" min-width="100">
      </el-table-column>
      ...
      <el-table-column label="操作" min-width="100">
        <template scope="scope">
          ...
          <el-button size="small" v-if="scope.row.fileStatus==2" @click="download(scope.row)">下载订单</el-button>
        </template>
      </el-table-column>
...
const queryAsyncJobsUrl = '/finance/report/queryRepFastFiles';
  ...
  methods: {
    searchHandler() {
      ...
      if (this.dateRange && this.dateRange[0] && this.dateRange[1]) {
        query.queryCondition.beginDate = dateFormat(this.dateRange[0], 'yyyy-mm-dd HH:MM:ss');
        query.queryCondition.endDate = dateFormat(this.dateRange[1], 'yyyy-mm-dd') + ' 23:59:59';
      }
      this.loading = true;
