<template>
  <div class="order-export-task">
    ...
    <el-table :data="taskData.beanList" style="width: 100%" v-loading="loading" element-loading-text="拼命加载中">
      ...
      <el-table-column prop="downloadCount" label="下载次数" class-name="count" min-width="100">
      </el-table-column>
      <el-table-column label="操作" min-width="100">
        <template scope="scope">
          ...
          <el-button size="small" v-if="scope.row.jobStatus==2" @click="download(scope.row)">下载订单</el-button>
        </template>
      </el-table-column>
...
export default {
  ...
  computed: {
    ...
    // taskData() {
    //   let beanList = this.taskOriginalData.beanList.map(n => {
    //     let beginTime, endTime, orderStatus;
    //     JSON.parse(n.queryCond).conditionType.forEach(m => {
    //       if (m.condType === 'beginPayTime') {
    //         beginTime = m.condValue[0];
    //       }
    //       if (m.condType === 'endPayTime') {
    //         endTime = m.condValue[0];
    //       }
    //       if (m.condType === 'orderStatus') {
    //         orderStatus = this.orderType[parseInt(m.condValue[0])].label;
    //       }
    //     });
    //     let queryTimeRange = (beginTime || '') + ' ~ ' + (endTime || '');
    //     return {
    //       orderStatus,
    //       queryTimeRange,
    //       ...n,
    ...
    //   });
    //   return {
    //     amount: this.taskOriginalData.amount,
    //     beanList,
    //   }
    // }
  },
