<template>
  <div class="OrderTapTwo">
    <div class="OrderTapTwo-header">
      ...
      <el-row class="echart" id="shop-chart-x">
      </el-row>
    </div>
    <div class="content-two">
      <h1 class="title-search">
        <span style='margin-right:20px'>所有店铺订单数据</span>
        <el-input size='mini' v-model.trim="shopName" placeholder="输入店铺名称" style='width:200px;margin-right:10px'></el-input>
        <el-date-picker
          size='mini'
          v-model="dateValue"
          type="daterange"
      ...
      <el-table
        ...
        <el-table-column
          ...
          label="平台">
          <template scope="scope">
            <span :class="icon(scope.row.platformId)"></span>
            <span v-text='platformName(scope.row.platformId)'></span>
          </template>
        </el-table-column>
        <el-table-column
          prop="orderNum"
...
let queryOrderShopTable = 'dataCenter/orderCenter/queryOrderShopTable'
...
export default {
  ...
  methods: {
    ...
    platformName(platformName) {
      return this.$store.state.enumSet.platformName[platformName]
    },
    refreshChart() {
      if (this.shopChart) {
        this.shopChart.resize();
        this.loadChartsData(this.shopChart, 'inCountAll', [this.activeShopStatType], orderChartsUrl);
      } else {
        this.shopChart = chartsInit('shop-chart-x');
        this.loadChartsData(this.shopChart, 'inCountAll', [this.activeShopStatType], orderChartsUrl);
      }
    },
    searchTabel() {
      ...
      this.queryCondition.shopName = this.shopName;
      ...
      this.queryOrderShopTable(this.queryCondition);
    },
    shopCheck(i) {
      this.shopSelect.forEach(ele => {
        ele.active = false;
    ...
    queryOrderShopTable(queryCondition) {
      ...
      this.axios.post(queryOrderShopTable, data).then(res => {
